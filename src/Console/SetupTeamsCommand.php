...
class SetupTeamsCommand extends Command
{
	 ...
	 * @var string
	 */
	protected $name = 'trust:setup-teams';
	...
	public function handle()
	{
		if (! Config::get('trust.teams.enabled')) {
			$this->error('Not using teams in your Trust configuration file.');
		...
		$this->line('');
		$this->info('The Trust teams feature setup is going to add a migration and a model');
	...
	protected function getExistingMigrationsWarning(array $existingMigrations)
	{
		if (count($existingMigrations) > 1) {
			$base = "Trust setup teams migrations already exist.\nFollowing files were found: ";
		} else {
			$base = "Trust setup teams migration already exists.\nFollowing file was found: ";
		}
