...
class Helper
{
	...
	public static function getIdFor($object, $type)
	{
		if (is_null($object)) {
			...
			return call_user_func_array([
				Config::get("trust.models.{$type}"), 'where',
	...
	{
		return Config::get('trust.foreign_keys.team');
	}
	...
	{
		if (is_null($team) || ! Config::get('trust.teams.enabled')) {
			return null;
		}
	...
	{
		if (
			! Config::get('trust.teams.enabled')
			|| (! Config::get('trust.teams.strict_check') && is_null($team))
		) {
			return true;
		}
	 ...
	 *
	 * @param  string|\Jetlabs\Trust\Models\TrustRole  $role
	 * @return bool
	 */
	public static function roleIsEditable($role)
	{
		...
		return ! in_array(
			$roleName,
			Config::get('trust.panel.roles_restrictions.not_editable') ?? []
		);
	}
	...
	public static function roleIsDeletable($role)
	{
		...
		return ! in_array(
			$roleName,
			Config::get('trust.panel.roles_restrictions.not_deletable') ?? []
		);
	}
	...
	public static function roleIsRemovable($role)
	{
		...
		return ! in_array(
			$roleName,
			Config::get('trust.panel.roles_restrictions.not_removable') ?? []
		);
	}
}
