...
class TrustUserQueryChecker extends TrustUserChecker
{
	/**
	...
	public function getCurrentUserRoles($team = null)
	{
		...
		if ($team === null) {
			return $this->user
				->roles()
				->wherePivot(config('trust.foreign_keys.team'), null)
				->pluck('name')
				->toArray();
		}
		...
		return $this->user
			->roles()
			->wherePivot(config('trust.foreign_keys.team'), $teamId)
			->pluck('name')
			->toArray();
	}
	...
	public function currentUserHasRole($name, $team = null, $requireAll = false)
	{
		...
		$teamStrictCheck = Config::get('trust.teams.strict_check');
		...
		$rolesCount = $this->user->roles()
			...
			->when($useTeams && ($teamStrictCheck || ! is_null($team)), function ($query) use ($team) {
				...
				return $query->where(Config::get('trust.foreign_keys.team'), $teamId);
			})
			->count();
