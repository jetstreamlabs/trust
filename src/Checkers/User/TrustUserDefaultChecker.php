...
namespace Jetlabs\Trust\Checkers\User;
...
class TrustUserDefaultChecker extends TrustUserChecker
{
    /**
    ...
    public function getCurrentUserRoles($team = null)
    {
        ...
        if (config('trust.teams.enabled') === false) {
            return $roles->pluck('name')->toArray();
        }

        if ($team === null && config('trust.teams.strict_check') === false) {
            return $roles->pluck('name')->toArray();
        }

        if ($team === null) {
            return $roles->filter(function ($role) {
                return $role['pivot'][config('trust.foreign_keys.team')] === null;
            })->pluck('name')->toArray();
        }
        ...
        return $roles->filter(function ($role) use ($teamId) {
            return $role['pivot'][config('trust.foreign_keys.team')] == $teamId;
        })->pluck('name')->toArray();
    }
    ...
    public function currentUserHasPermission($permission, $team = null, $requireAll = false)
    {
        ...
        foreach ($this->userCachedRoles() as $role) {
            $role = Helper::hidrateModel(Config::get('trust.models.role'), $role);
    ...
    protected function userCachedRoles()
    {
        ...
        if (!Config::get('trust.cache.enabled')) {
            return $this->user->roles()->get();
        }

        return Cache::remember($cacheKey, Config::get('trust.cache.expiration_time', 60), function () {
    ...
    {
        foreach (Config::get('trust.user_models') as $key => $model) {
