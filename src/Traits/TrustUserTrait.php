...
trait TrustUserTrait
{
	 ...
	 * @return void|bool
	 */
	public static function bootTrustUserTrait()
	{
	...
	public function roles()
	{
		...
		if (Config::get('trust.teams.enabled')) {
			$roles->withPivot(Config::get('trust.foreign_keys.team'));
		}
	...
	public function rolesTeams()
	{
		...
		return $this->morphToMany(
			Config::get('trust.models.team'),
			'user',
			Config::get('trust.tables.role_user'),
			Config::get('trust.foreign_keys.user'),
			Config::get('trust.foreign_keys.team')
		)
			->withPivot(Config::get('trust.foreign_keys.role'));
	}
	...
	public function permissionsTeams()
	{
		...
		return $this->morphToMany(
			...
			->withPivot(Config::get('trust.foreign_keys.permission'));
	}
	...
	public function permissions()
	{
		...
		if (Config::get('trust.teams.enabled')) {
			$permissions->withPivot(Config::get('trust.foreign_keys.team'));
		}
	 ...
	 *
	 * @return \Jetlabs\Trust\Checkers\User\TrustUserChecker
	 */
	protected function laratrustUserChecker()
	{
		return (new TrustCheckerManager($this))->getUserChecker();
	}
	...
	private function attachModel($relationship, $object, $team)
	{
		...
		if (Config::get('trust.teams.enabled')) {
			...
			if (
					...
					->wherePivot(Config::get("trust.foreign_keys.{$objectType}"), $object)
					->count()
				) {
				return $this;
			}
		...
		$this->flushCache();
		$this->fireTrustEvent("{$objectType}.attached", [$this, $object, $team]);
	...
	private function detachModel($relationship, $object, $team)
	{
		...
		$this->flushCache();
		$this->fireTrustEvent("{$objectType}.detached", [$this, $object, $team]);
	...
	private function syncModels($relationship, $objects, $team, $detaching)
	{
		...
		$objectType = Str::singular($relationship);
		$mappedObjects = [];
		$useTeams = Config::get('trust.teams.enabled');
		...
		$this->flushCache();
		$this->fireTrustEvent("{$objectType}.synced", [$this, $result, $team]);
	...
	{
		if ($thing instanceof \Jetlabs\Trust\Contracts\Ownable) {
	...
	public function allPermissions($columns = null, $team = false)
	{
		...
		$roles = $this->roles()
			->when(config('trust.teams.enabled') && $team !== false, function ($query) use ($team) {
				return $query->whereHas('permissions', function ($permissionQuery) use ($team) {
					$permissionQuery->where(config('trust.foreign_keys.team'), Helper::getIdFor($team, 'team'));
				});
			})
		...
		$directPermissions = $this->permissions()
			->when(config('trust.teams.enabled') && $team !== false, function ($query) use ($team) {
				$query->where(config('trust.foreign_keys.team'), Helper::getIdFor($team, 'team'));
			});
	...
	{
		$case = str_replace('_case', '', Config::get('trust.magic_is_able_to_method_case'));
